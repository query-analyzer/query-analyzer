...
the same stack; per-scenario verdicts and raw confusion counts are in the
artifact.}
\label{tab:accuracy}
\small
...
\midrule
\tool{} (all three modes)     & \textbf{1.00} & \textbf{1.00} & \textbf{1.00} \\
JPlusOne 2.0.0 (live)         & 1.00 & 0.50 & 0.67 \\
Count-based baseline          & 0.40 & 1.00 & 0.57 \\
\bottomrule
\end{tabular}
\vspace{-4pt}
\end{table}
...
%% ===========================================================================
\tool{} is open-source (MIT). Source code, the benchmark module, the
PetClinic case study, and
reproduction instructions are at
\url{https://github.com/query-analyzer/query-analyzer}; released binaries are on
Maven Central (\code{io.github.query-analyzer}, v1.2.5).
%% TODO(camera-ready): replace the two URLs below before Aug 7.
%%   - VIDEO-URL: unlisted YouTube or similar link to the ~3 min demo.
%%   - ARCHIVE-DOI: archived snapshot (e.g. a Zenodo DOI from the GitHub release).
A demonstration video is at \url{VIDEO-URL} and an archived artifact snapshot at
\url{ARCHIVE-DOI}.
