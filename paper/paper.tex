%% ---------------------------------------------------------------------------
%% Query Analyzer -- SPLASH Companion '26, Tool Demonstrations.
%% Rights metadata below is as supplied by ACM's Publication Release
%% Confirmation and should not be edited by hand.
%% ---------------------------------------------------------------------------
...
\setcopyright{cc}
\setcctype{by}
\acmConference[SPLASH Companion '26]{Companion Proceedings of the 2026 ACM SIGPLAN International Conference on Systems, Programming, Languages, and Applications: Software for Humanity}{October 03--09, 2026}{Oakland, CA, USA}
\acmBooktitle{Companion Proceedings of the 2026 ACM SIGPLAN International Conference on Systems, Programming, Languages, and Applications: Software for Humanity (SPLASH Companion '26), October 03--09, 2026, Oakland, CA, USA}
\acmDOI{10.1145/3837729.3840484}
...
% Extra tolerance so the long URLs do not overflow the column.
...
\orcid{0009-0000-5098-9775}
\email{mahmoud.mahmoud03@eng-st.cu.edu.eg}
\affiliation{%
...
costly performance defects in data-access code, yet tool support on the Java
Virtual Machine (JVM) is fragmented. Raw Java Database Connectivity (JDBC)
interceptors log statements but do not detect anti-patterns;
...
open-source tool that automatically detects N+1 queries, analyzes their query
plans, and suggests fixes. It intercepts queries at the JDBC layer,
so it works across frameworks (Hibernate, Spring Data JPA, MyBatis, jOOQ, plain
...
%% ===========================================================================
Modern Java Virtual Machine (JVM) applications rarely write SQL by hand. Object-relational mapping
...
datasource-proxy~\cite{datasourceproxy}) transparently log or count Java
Database Connectivity (JDBC) statements, but leave \emph{detection} of
anti-patterns entirely to the
developer.
\item \textbf{Test-time assertion libraries} (QuickPerf~\cite{quickperf},
JPlusOne~\cite{jplusone}, the Hypersistence Utils statement-count
validator~\cite{hypersistenceutils},
...
false-positive suppression (pagination, polling, batched \code{IN},
sub-threshold repetition) that distinguishes true N+1s from legitimate
repetition, where count-based detectors fail.
\item A \code{@NoNPlusOne} JUnit~5 extension that turns detection into a
continuous integration (CI) \textbf{regression guard}, plus
\texttt{EXPLAIN}-based query-plan analysis and
...
\Description{Three lines of Java. The first calls userRepository.findAll(),
annotated as one query. A for loop over the returned users then calls
u.getOrders().size() in its body, annotated as issuing N additional lazy
queries, giving N+1 queries in total.}
\label{fig:motiv}
\vspace{-6pt}
\end{figure}
...
shape --- paginated scans, steady-cadence polling, batched \code{IN (...)}
fetches, or the same lookup issued from several services in one request. A tool
that cries wolf on these trains developers to ignore it. \tool{} separates these
cases from genuine N+1s without a hand-specified expected count per test.
...
groups that match known-legitimate patterns. Each of these repeats a query shape
for a benign reason that a count-based rule would misread as an N+1, so
suppressing them is what keeps the tool from crying wolf: (i) \emph{pagination},
repeated reads of one shape that all carry an \code{OFFSET} clause, including the
common case where the offset is a bind parameter (\code{OFFSET ? ROWS FETCH ...})
and no literal is visible --- these walk successive pages of one result set
rather than issuing one query per parent row; (ii) \emph{streaming/polling}, a
timestamp predicate read at a steady cadence --- a query deliberately re-run on a
timer, not fired inside a loop over a collection; (iii) \emph{batched}
\code{IN (...)} fetches --- already the single batched query that \emph{fixes} an
N+1, not the defect; and (iv) sub-threshold repetition --- too few repeats to
indicate a loop. Crucially, a bare repeated \code{LIMIT n} with no offset
...
\begin{tikzpicture}[
  ...
  verdict/.style={draw, rounded corners, align=center, inner sep=2.5pt,
...
\node[verdict, right=7mm of dec, yshift=4.5mm, fill=black!78, text=white] (flag) {N+1};
\node[verdict, right=7mm of dec, yshift=-4.5mm] (pass) {legit.};
\draw[->] (sf.east) -- node[above,sloped,font=\tiny]{$w_f{=}.5$} (sum);
\draw[->] (st.east) -- node[above,font=\tiny,pos=.55]{$w_t{=}.2$} (sum);
...
\Description{Three input boxes labelled framework signal S-f (ORM and
lazy-load frames), timing signal S-t (loop tightness), and location signal S-l
(call-site proximity) feed into a summation node with weights 0.5, 0.2, and 0.3
respectively. The sum S flows into a decision diamond testing whether S is at
least theta; the yes branch outputs a verdict of N+1 and the no branch outputs
legitimate.}
...
in a \code{query-analyzer-benchmark} module and run with one command
(\code{./run-all.sh}); numbers below are from a commodity laptop. The controlled benchmark runs on H2,
...
addresses. These baselines target somewhat different tasks: query-budget
libraries such as Hypersistence Utils enforce a \emph{developer-specified}
statement count rather than inferring N+1s. We therefore report what each
concludes without a hand-specified count --- measuring which tools separate N+1s
from legitimate repetition unaided, not a like-for-like contest.

\noindent\textbf{Feature comparison.} Table~\ref{tab:features} situates \tool{}
against representative tools.
...
recall, and F1 on the seven-scenario suite (2 true N+1, 5 clean; see
Limitations). \tool{} flags both N+1s and none of the five clean cases
...
(0.50 recall, 0.67~F1), and it has no dedicated production reporting mode. The live Hypersistence counter returns raw statement
...
Because aggregate scores over seven cases reveal little on their own,
Table~\ref{tab:perscenario} reports the verdict every detector reached on every
scenario, so each of the four errors above---JPlusOne's one missed N+1 and the
baseline's three false alarms---is attributable to a specific named workload
rather than inferred from a summary statistic.

\begin{table}[t]
\centering
\caption{Detection accuracy on the labelled suite (7 scenarios; 2 true N+1, 5
clean). JPlusOne and the count-based (QuickPerf-style) baseline are run live on
the same stack. Table~\ref{tab:perscenario} gives the per-scenario breakdown.}
\label{tab:accuracy}
\small
...
\midrule
\tool{} (all modes)  & 2 & 0 & 5 & 0 & \textbf{1.00} & \textbf{1.00} & \textbf{1.00} \\
JPlusOne 2.0.0       & 1 & 0 & 5 & 1 & 1.00 & 0.50 & 0.67 \\
Count-based baseline & 2 & 3 & 2 & 0 & 0.40 & 1.00 & 0.57 \\
\bottomrule
\end{tabular}
\vspace{-4pt}
\end{table}
...
\begin{table}[t]
\centering
\caption{Per-scenario verdicts behind Table~\ref{tab:accuracy}. Each row is one
labelled workload; $|T|$ is its captured trace length in statements, and each
detector column is the verdict that detector reached on that identical trace.
\colorbox{gray!25}{Shaded} cells disagree with ground truth. \tool{}'s
\textsc{Threshold}, \textsc{Confidence}, and \textsc{Hybrid} modes agree on
every scenario, so they share one column (QA).}
\label{tab:perscenario}
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{llcccc}
\toprule
Scenario & Truth & $|T|$ & QA & JPlusOne & Count \\
\midrule
Classic N+1 (lazy collection)  & N+1   & 6 & N+1   & N+1 & N+1 \\
N+1 via query method           & N+1   & 6 & N+1   & \cellcolor{gray!25}clean & N+1 \\
\texttt{JOIN FETCH} (optimized) & clean & 1 & clean & clean & clean \\
Pagination loop                & clean & 5 & clean & clean & \cellcolor{gray!25}N+1 \\
Streaming poll (timestamp)     & clean & 5 & clean & clean & \cellcolor{gray!25}N+1 \\
Batched \texttt{IN} clause      & clean & 1 & clean & clean & clean \\
Repeated below threshold       & clean & 2 & clean & clean & \cellcolor{gray!25}N+1 \\
\bottomrule
\end{tabular}
\vspace{-6pt}
\end{table}
...
Here a pure threshold raises a false positive (precision 0.60) while
...
Sweeping the decision threshold $\theta$ (Fig.~\ref{fig:sensitivity}), F1 peaks across $0.4$--$0.5$, justifying the choice of $\theta=0.5$. A grid sweep of the weights keeps F1 in $[0.75,1.00]$, with the default $(0.5,0.2,0.3)$ among the settings reaching the optimal F1${=}1.00$ (full grids in the artifact).
...
\Description{A line chart with the confidence threshold theta from 0.1 to 0.9 on
the horizontal axis and score from 0.5 to 1.0 on the vertical axis. Precision
rises from 0.60 to 1.00 at theta equal to 0.4 and stays at 1.00. Recall holds at
1.00 through theta equal to 0.5 then drops to 0.67. F1 is 0.75 for low
thresholds, reaches 1.00 across theta of 0.4 to 0.5, and falls to 0.80
thereafter, so F1 peaks on the interval 0.4 to 0.5.}
...
unrelated code, applying \tool{} to a
...
realistic DB-backed endpoints. On the same instance the tool's \texttt{EXPLAIN} analyzer correctly
...
fixed per-request figure, not a percentage guarantee.
...
%% ===========================================================================
\tool{} is open-source (MIT). For \emph{reproduction}, the archived
package accompanying this paper---tool, benchmark module, PetClinic case
study, and one-command reproduction instructions---is deposited on
Zenodo~\cite{queryanalyzer-artifact}; that fixed deposit is the one all
numbers reported here were produced with. For \emph{reuse} and
continued development, the project is at
\url{https://github.com/query-analyzer/query-analyzer}, with released
binaries on Maven Central (\code{io.github.query-analyzer}). A
demonstration video is at
\url{https://www.youtube.com/watch?v=10EykUiJ7To}.
